...
def main(args):
    ...
    if not encoder:
        print("ERROR: Could not create 'x264enc' element.")
        sys.exit(1)
    encoder.set_property('bitrate', 2000000)

    muxer = Gst.ElementFactory.make("qtmux", "qt-muxer")
    if not muxer:
        print("ERROR: Could not create 'qtmux' element.")
        sys.exit(1)
    ...
    sink.set_property("location", "./output3.mp4")
    ...
    if not nvosd.link(nvvidconv2):
        print("ERROR: Could not link nvosd to nvvidconv2.")
        sys.exit(1)

    # if not nvvidconv2.link(capsfilter):
    #     print("ERROR: Could not link nvosd to capsfilter.")
    #     sys.exit(1)

    if not nvvidconv2.link(encoder):
        print("ERROR: Could not link capsfilter to encoder.")
        sys.exit(1)
    # nvosd.link(capsfilter)
    # capsfilter.link(encoder)
    encoder.link(codeparser)
    # sinkpad1 = muxer.get_request_pad("video_0")
    # if not sinkpad1:
